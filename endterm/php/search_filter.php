...
ini_set("error_log", "/tmp/php-error.log");
    ...
    if(isset($_POST["action"])){
        ...
        if(isset($_POST["dept_time_back"]) && !empty($_POST["dept_time_back"])){
            ...
            $result_there = mysqli_query($db, $query_there);
            ...
            if($total_row_there > 0){
                while($row = mysqli_fetch_array($result_there)){
                    ...
                    $result_back = mysqli_query($db, $query_temp_back);
                    $row_back = mysqli_num_rows($result_back);
                    $total_row_back += $row_back;
                    if($row_back > 0){
                        while($row_back = mysqli_fetch_array($result_back)){
                            $query_airline_there = "SELECT image FROM airlines WHERE id = ".$row['airline_id']."";
                            $query_airline_back = "SELECT image FROM airlines WHERE id = ".$row_back['airline_id']."";
                            $result_airline_there = mysqli_query($db, $query_airline_there);
                            $result_airline_back = mysqli_query($db, $query_airline_back);
                            $airline_name_there = mysqli_fetch_array($result_airline_there)["image"];
                            $airline_name_back = mysqli_fetch_array($result_airline_back)["image"];
                            ...
                            <div class='ticket-card'>
                                <div class='ticket-card-airline'>
                                    <img src='images/airlines/".$airline_name_there."' width='120px'>
                                    <img src='images/airlines/".$airline_name_back."' width='120px'>
                                </div>
                                <div class='ticket-card-transit'>
                                    <p class='ticket-time-interval-text-1'>прямой</p>
                                </div>
                                ...
                                <div class='ticket-card-time-interval'>
                                    <p class='ticket-time-interval-text-1'>".$hours_there." ч ".$minutes_there." м</p>
                                    <p class='ticket-time-interval-text-1'>".$hours_back." ч ".$minutes_back." м</p>
                                </div>
            ...
            if($total_row_there > 0){
                while($row = mysqli_fetch_array($result_there)) {
                    ...
                    <div class='ticket-card'>
                        ...
                        <div class='ticket-card-time-interval'>
                            <p class='ticket-time-interval-text'>".$hours_there." ч ".$minutes_there." м</p>
                        </div>
                        <div class='ticket-card-buy-btn-block'>
